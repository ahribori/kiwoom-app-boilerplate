...
class Kiwoom(QObject):
    ...
    OnReceiveMsg = pyqtSignal([str, str, str, str], ['QString', 'QString', 'QString', 'QString'])
    OnReceiveTrData = pyqtSignal([str, str, str, str, str],
                                 ['QString', 'QString', 'QString', 'QString', 'QString'])
    OnReceiveRealData = pyqtSignal([str, str, str], ['QString', 'QString', 'QString'])
    OnReceiveChejanData = pyqtSignal([str, int, str], ['QString', 'QString', 'QString'])
    OnReceiveConditionVer = pyqtSignal([int, str], ['QString', 'QString'])
    OnReceiveTrCondition = pyqtSignal([str, str, str, int, int],
                                      ['QString', 'QString', 'QString', 'QString', 'QString'])
    OnReceiveRealCondition = pyqtSignal([str, str, str, str], ['QString', 'QString', 'QString', 'QString'])
    ...
    def __init__(self):
        ...
        self.api.OnReceiveTrData[str, str, str, str, str, int, str, str, str].connect(self._OnReceiveTrData)
        self.api.OnReceiveRealData[str, str, str].connect(self._OnReceiveRealData)
        self.api.OnReceiveChejanData[str, int, str].connect(self._OnReceiveChejanData)
        self.api.OnReceiveConditionVer[int, str].connect(self._OnReceiveConditionVer)
        self.api.OnReceiveTrCondition[str, str, str, int, int].connect(self._OnReceiveTrCondition)
        self.api.OnReceiveRealCondition[str, str, str, str].connect(self._OnReceiveRealCondition)
    ...
    def _OnEventConnect(self, errCode):
        self.OnEventConnect.emit(errCode)

    # 수신 메시지 이벤트
    def _OnReceiveMsg(self, scrNo, rQName, trCode, msg):
        self.OnReceiveMsg.emit(scrNo, rQName, trCode, msg)
    ...
    def _OnReceiveTrData(self, scrNo, rQName, trCode, recordName, prevNext, dataLength, errorCode, message, splmMsg):
        ...
        self.OnReceiveTrData.emit(scrNo, rQName, trCode, recordName, prevNext)

    # 실시간 시세 이벤트
    def _OnReceiveRealData(self, jongmokCode, realType, realData):
        self.OnReceiveRealData.emit(jongmokCode, realType, realData)

    # 체결데이터를 받은 시점을 알려준다.
    # sGubun – 0:주문체결통보, 1:잔고통보, 3:특이신호
    # sFidList – 데이터 구분은 ‘;’ 이다.
    def _OnReceiveChejanData(self, gubun, itemCnt, fidList):
        self.OnReceiveChejanData.emit(gubun, itemCnt, fidList)

    # 로컬에 사용자조건식 저장 성공여부 응답 이벤트
    def _OnReceiveConditionVer(self, ret, msg):
        self.OnReceiveConditionVer.emit(ret, msg)
    ...
    def _OnReceiveTrCondition(self, scrNo, codeList, conditionName, index, next):
        self.OnReceiveTrCondition.emit(scrNo, codeList, conditionName, index, next)
    ...
    def _OnReceiveRealCondition(self, code, type, conditionName, conditionIndex):
        self.OnReceiveRealCondition(code, type, conditionName, conditionIndex)

    # 로그인
    # 0 - 성공, 음수값은 실패
    @pyqtSlot(result=int)
    def commConnect(self):
        return self.api.dynamicCall("CommConnect()")
    ...
    def getLoginInfo(self, tag):
        return self.api.dynamicCall("GetLoginInfo(QString)", [tag])
    ...
    def commRqData(self, rQName, trCode, prevNext, screenNo):
        return self.api.dynamicCall("CommRqData(QString, QString, int, QString)", rQName, trCode, prevNext, screenNo)

    # 수신 받은 데이터의 반복 개수를 반환한다.
    @pyqtSlot(str, str, result=int)
    def getRepeatCnt(self, trCode, recordName):
        return self.api.dynamicCall("GetRepeatCnt(QString, QString)", trCode, recordName)
    ...
    def commGetData(self, jongmokCode, realType, fieldName, index, innerFieldName):
        return self.api.dynamicCall("CommGetData(QString, QString, QString, int, QString)", jongmokCode, realType,
    ...
    def getCommRealData(self, realType, fid):
        return self.api.dynamicCall("GetCommRealData(QString, int)", realType, fid).strip()
    ...
    def sendOrder(self, rQName, screenNo, accNo, orderType, code, qty, price, hogaGb, orgOrderNo):
        return self.api.dynamicCall("SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                                    [rQName, screenNo, accNo, orderType, code, qty, price, hogaGb, orgOrderNo])

    # 체결잔고 데이터를 반환한다.
    @pyqtSlot(int, result=str)
    def getChejanData(self, fid):
        return self.api.dynamicCall("GetChejanData(int)", fid)

    # 서버에 저장된 사용자 조건식을 가져온다.
    @pyqtSlot(result=int)
    def getConditionLoad(self):
        return self.api.dynamicCall("GetConditionLoad()")
    ...
    def getConditionNameList(self):
        return self.api.dynamicCall("GetConditionNameList()")
    ...
    def sendCondition(self, scrNo, conditionName, index, search):
        self.api.dynamicCall("SendCondition(QString,QString, int, int)", scrNo, conditionName, index, search)
    ...
    def sendConditionStop(self, scrNo, conditionName, index):
        self.api.dynamicCall("SendConditionStop(QString, QString, int)", scrNo, conditionName, index)
    ...
    # 	self.api.dynamicCall("CommKwRqData(QString, QBoolean, int, int, QString, QString)", arrCode, next, codeCount, typeFlag, rQName, screenNo)
    ...
    def setRealReg(self, screenNo, codeList, fidList, optType):
        return self.api.dynamicCall("SetRealReg(QString, QString, QString, QString)", screenNo, codeList, fidList,
                                    optType)
    ...
    def setRealRemove(self, scrNo, delCode):
        self.api.dynamicCall("SetRealRemove(QString, QString)", scrNo, delCode)
    ...
    def getCommDataEx(self, trCode, recordName):
        return json.dumps(self.api.dynamicCall("GetCommDataEx(QString, QString)", trCode, recordName))
    ...
    def disconnectRealData(self, scnNo):
        self.api.dynamicCall("DisconnectRealData(QString)", scnNo)
    ...
    def getMasterCodeName(self, code):
        return self.api.dynamicCall("GetMasterCodeName(QString)", code)
    ...
    def getCodeListByMarket(self, strMarket):
        return self.api.dynamicCall("GetCodeListByMarket(QString)", strMarket)

    # 입력한 종목의 전일가를 전달
    # strCode – 종목코드
    def getMasterLastPrice(self, code):
        return self.api.dynamicCall("GetMasterLastPrice(QString)", code)
